...
export default function useChatPage() {
	...
	const fetchChunk = useCallback(
		async (newOffset = 0) => {
			...
			try {
				...
				setMessages((prev) => (newOffset === 0 ? mapped : [...mapped, ...prev].slice(-DISPLAY_MESSAGE_LIMIT)));
				setOffset(newOffset);
	...
	useEffect(() => {
		...
		if (authState.loggedIn && socket) {
			...
			socket.on("message_sent", (_id, msgs) => {
				...
				if (!last) return;
				setMessages((prev) => [...prev, ...mapMessages([last])].slice(-DISPLAY_MESSAGE_LIMIT));
			});
	...
	}, [authState.loggedIn, authState.loggingIn, authState.socketInfo.currentRoom, authState.userId, authState.socketInfo.connected, dispatch, fetchChunk]);
