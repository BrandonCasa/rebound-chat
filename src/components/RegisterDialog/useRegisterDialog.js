...
const useRegisterDialog = () => {
  ...
  const registerDialogState = useSelector(
    (state) => state.dialogs.registerDialogOpen,
  );
  ...
  const validateEmail = (value) => {
    ...
    if (
      ...
      !value.toString().split("@")[1].includes(".")
    ) {
  ...
  const handleFormDataChange = (field, value, _errors = {}) => {
    setFormData({
      ...
      [field]: value,
      bioErrors:
        field === "bio" ? { ...validateBio(value) } : { ...formData.bioErrors },
      emailErrors:
        field === "email"
          ? { ...validateEmail(value) }
  ...
  const handleUserRegister = async () => {
    ...
    requestString =
      process.env.NODE_ENV !== "development" && isElectron
        ? `https://rebound.nexus/api/users/register`
        : requestString;
  ...
  const handleDialogClose = () => {
    dispatch(
      setDialogOpened({ dialogName: "registerDialogOpen", newState: false }),
    );
    setFormData({
      bio: formData.bio,
      email: formData.email,
  ...
  const errorMessages = [
    ...
    formData.usernameErrors.short &&
      "Username must be at least 3 characters long.",
    formData.usernameErrors.long &&
    ...
    formData.passwordErrors.short &&
      "Password must be at least 5 characters long.",
    formData.passwordErrors.long &&
