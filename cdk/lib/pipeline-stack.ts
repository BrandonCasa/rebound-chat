	...
	constructor(scope: Construct, id: string, props: PipelineStackProps) {
		...
		const projects = {
			...
			frontend: this.createProject("FrontendBuild", "frontend", "infra/buildspec.frontend.yml", config, {
				...
				FRONTEND_BUCKET_NAME: props.frontendBucketName,
		...
		projects.frontend.role?.addToPrincipalPolicy(
			new iam.PolicyStatement({
				actions: ["s3:Abort*", "s3:DeleteObject*", "s3:PutObject", "s3:PutObjectLegalHold", "s3:PutObjectRetention", "s3:PutObjectTagging", "s3:PutObjectVersionTagging"],
		...
		projects.frontend.role?.addToPrincipalPolicy(
			new iam.PolicyStatement({
				...
				resources: [`arn:aws:cloudfront::${Stack.of(this).account}:distribution/${props.cloudFrontDistributionId}`],
			})
		);
