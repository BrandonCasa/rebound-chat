import { expect, request, registerUser, loginUser, extractCookie, fetchCsrfToken, createBackend, resetUsers } from "./helpers/authTestUtils.js";
...
describe("Token refresh", () => {
	...
	it("requires a valid refresh token cookie", async () => {
		const agent = request.agent(backend.server);
		const missingTokenCsrf = await fetchCsrfToken(agent);

		const missingTokenRes = await agent.post("/api/users/refresh").set("x-csrf-token", missingTokenCsrf);
		...
		const tamperedAgent = request.agent(backend.server);
		const tamperedTokenRes = await tamperedAgent
			.post("/api/users/refresh")
			.set("Cookie", [`jid=invalid-token`, `csrfToken=${csrfToken}`].join("; "))
			.set("x-csrf-token", csrfToken);
