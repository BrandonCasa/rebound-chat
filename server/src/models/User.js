...
UserSchema.methods.toProfilePubJSON = async function (
  ...
  const theirConfirmed = queryingData.friends
    .filter((f) => f.confirmed)
    .map((f) =>
      f.requester.toString() === queryingData._id.toString()
        ? f.recipient.toString()
        : f.requester.toString(),
    );

  const mutualFriendIds = myConfirmed.filter((id) =>
    theirConfirmed.includes(id),
  );
  ...
  const mutualServers = this.servers.filter((s) =>
    theirServers.includes(s.toString()),
  );
