...
export const getUser = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select(privateFieldsString);
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
...
export const setDisplayName = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Display name changed.', user: newUser });
  } catch (err) {
...
export const setInterests = async (req, res) => {
  try {
    ...
    const newUser = await User.findByIdAndUpdate(
      _id,
      { interests: req.body.newinterests },
      { new: true }
    );
    return res
      .status(200)
...
export const cleanFriendRequests = async (_id) => {
  try {
    ...
    const newFriendRequestsIn = [];
    const newFriendRequestsOut = [];
    for (let i = 0; i < user.friendRequestsIn.length; i += 1) {
    ...
    for (let i = 0; i < user.friendRequestsOut.length; i += 1) {
    ...
    const newUser = await User.findByIdAndUpdate(
      _id,
      {
        friendRequestsIn: newFriendRequestsIn,
        friendRequestsOut: newFriendRequestsOut,
      },
      { new: true }
    );
    return newUser;
...
export const addFriend = async (req, res) => {
  try {
    ...
    const newUser = await User.findByIdAndUpdate(
      _id,
      { friendRequestsOut: [...user.friendRequestsOut, friendId] },
      { new: true }
    );
    await User.findByIdAndUpdate(
      friendId,
      { friendRequestsIn: [...friend.friendRequestsIn, _id.toString()] },
      { new: true }
    );
    return res
      .status(200)
      .json({ message: 'Friend request sent', user: newUser });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
export const acceptFriendRequest = async (req, res) => {
  try {
    ...
    const newUser = await User.findByIdAndUpdate(
      _id,
      {
        friendRequestsIn: user.friendRequestsIn.filter((id) => id !== friendId),
    ...
    await User.findByIdAndUpdate(
      friendId,
      {
        friendRequestsOut: friend.friendRequestsOut.filter(
          (id) => id !== _id.toString()
        ),
        friends: [...friend.friends, _id.toString()],
      },
      { new: true }
    );
    return res
      .status(200)
      .json({ message: 'Friend request accepted', user: newUser });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
export const declineFriendRequest = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Friend request declined', user: newUser });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
export const cancelFriendRequest = async (req, res) => {
  try {
    ...
    if (!user.friendRequestsOut.includes(friendId)) {
      return res
        .status(400)
        .json({ message: 'You do not have a friend request to this user' });
    }
    const newUser = await User.findByIdAndUpdate(
      _id,
      {
        friendRequestsOut: user.friendRequestsOut.filter(
          (id) => id !== friendId
        ),
      },
      { new: true }
    ...
    await User.findByIdAndUpdate(
      friendId,
      {
        friendRequestsIn: friend.friendRequestsIn.filter(
          (id) => id !== _id.toString()
        ),
      },
      { new: true }
    ...
    return res
      .status(200)
      .json({ message: 'Friend request canceled', user: newUser });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
...
export const removeFriend = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(
      friendId,
      { friends: friend.friends.filter((id) => id !== _id.toString()) },
      { new: true }
    );
