...
const hasPasswordChangedAfterTokenIssue = (user, decoded) => {
	const passwordChangedAt = user?.passwordChangedAt instanceof Date ? user.passwordChangedAt.getTime() : null;
...
const parseUserAgentDetails = (userAgentRaw) => {
	...
	const deviceName = device?.model || device?.vendor ? [device?.vendor, device?.model].filter(Boolean).join(" ") : osName;
